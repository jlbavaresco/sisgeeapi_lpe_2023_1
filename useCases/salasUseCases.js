...
const Sala = require("../entities/salas")

const getSalasDB = async () => {
    try {
        const { rows } = await pool.query(`select s.codigo as codigo, 
        s.numero as numero, s.descricao as descricao,
        ...
        return rows.map((sala) => new Sala(sala.codigo, sala.numero, 
            sala.descricao,
            sala.capacidade, sala.predio, sala.nomepredio));
    } catch (err) {
        throw "Erro: " + err;
    }
}
...
const addSalaDB = async (body) => {
    try {
        ...
        const results = await pool.query(`INSERT INTO salas 
        (numero, descricao, capacidade, predio) values ($1, $2, $3, $4) RETURNING 
        codigo, numero, descricao, capacidade, predio`, 
        [numero, descricao, capacidade, predio]);
        const sala = results.rows[0];
        return new Sala(sala.codigo, sala.numero, sala.descricao,
            sala.capacidade, sala.predio, sala.nomepredio);
    } catch (err) {
...
const updateSalaDB = async (body) => {
    try {
        ...
        descricao=$2, capacidade=$3, predio=$4 WHERE codigo=$5 RETURNING 
        codigo, numero, descricao, capacidade, predio`, 
        ...
        throw "Erro ao atualizar a sala: " + err;
    }
}
...
const getSalaPorCodigoDB = async (codigo) => {
    try {
        ...
        throw "Erro ao recuperar o sala: " + err;
    }
}

module.exports = {
    getSalasDB, addSalaDB, updateSalaDB, deleteSalaDB,
    getSalaPorCodigoDB
}
